...
<div class="white-box">
    <div class="col-mod-12">
        ...
        {{-- <div class="col-mod-6 col-lg-6 ">
            <a href="{{ route('add_product') }}" class="waves-effect pull-right"><button
            class="btn btn-xs btn-info pull-right"><i class="fa fa-arrow-circle-left"></i> ADD NEW PRODUCT</button></a>
    </div> --}}
</div>
<div class="clear"></div>
<hr />
    ...
    <table id="myTable" class="table table-bordered table-striped dataTable no-footer" role="grid"
        ...
        <tbody>
            ...
            <tr role="row" class="odd">
                <td><a
                        href="{{route('admin-service-details', ['id' => $data->whichService->id])}}">{{ $data->whichService->title }}</a>
                </td>
                <td><a
                        href="{{route('user-details', ['id' => $data->whichService->createdBy->id])}}">{{ $data->whichService->createdBy->name }}</a>
                </td>
                <td><a href="{{route('user-details', ['id' => $data->reserver->id])}}">{{ $data->reserver->name }}</a>
                </td>
                <?php
