				...
				<div class="card-block">
					...
					<form action="" method="post">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="number" placeholder="Number" name="number" value="{{$details->number}}">
							<small class="text-danger">{{ $errors->first('number') }}</small>
						</div>
						
						<div class="form-group">
							<label for="cvv">CVV</label>
							<input type="text" class="form-control" id="cvv" placeholder="CVV" name="cvv" value="{{$details->cvv}}">
						...
						<div class="form-group">
							...
							<select name="exp_month" class="form-control">
								@for($i=1; $i<=12; $i++)
							  		<option value="{{$i}}" @if($details->exp_month==$i) selected="selected" @endif>{{$i}}</option>
								@endfor
							</select>
						...
						<div class="form-group">
							...
							<select name="exp_year" class="form-control">
								@for($i=date('Y'); $i<=date('Y')+10; $i++)
							  		<option value="{{$i}}" @if($details->exp_year==$i) selected="selected" @endif>{{$i}}</option>
								@endfor
							</select>
