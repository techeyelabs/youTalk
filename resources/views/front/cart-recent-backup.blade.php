						...
						<div class="col-md-12 p-0 mb-4">
							...
							<div class="row justify-content-center mt-5">
								<div class="col-md-12">
									<table class="table">
										<tr class="bg-dark">
											<th class="text-center">商品名</th> <th class="text-center" colspan="2">数量</th>  <th class="text-center">必要ポイント</th>
										</tr>
											...
											<tr>
												<td style="" class="">
													<div class="d-flex flex-row">
														...
														<span class="px-2">
															{{ $product->title }} <br>
															M／ネイビー <br> pdoduct->pont ポイント
														</span>
													</div>
												</td>
												<td class="text-center" >
													<div class="d-flex flex-row pt-5 justify-content-center">
														<button class="px-3 py-1 align-self-end border text-center bg-light decrease_btn" data-rowid="{{ $p->rowId }}">-</button>
														<form class="cart-item" action="{{route('front-cart-update')}}" method="get">
															<input type="hidden" name="row_id" value="{{$p->rowId}}">
															<input type="text" class="px-3 py-1 border align-self-start text-center cart_qty_{{ $p->rowId }}" name="quantity" value="{{ $p->qty }}" style="width:50px;">
														</form>
														<button class="px-3 py-1 align-self-end border text-center bg-light increase_btn" data-rowid="{{ $p->rowId }}">+</button>
													</div>
												</td>
												<td style="width:100px;" text-align="center" class="text-center">
													<div class="pt-5">
														<h4 style="letter-spacing:1px;">{{ $p->qty*$p->price }}P</h4>
													</div>
												</td>
											</tr>
										@endforeach
										<tr class="">
											...
											<td colspan="" class="text-center"><h5>{{Cart::count()}}</h5></td>
											<td class="text-center">合計ポイント</td>
											<td class="text-center"> <h5 class="text-danger">{{Cart::subtotal()}} P</h5></td>
